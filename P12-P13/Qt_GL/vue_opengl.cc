...
void VueOpenGL::dessine(Systeme const& a_dessiner){
    ...
    glClearColor(0.83, 0.9, 1, 1); //fond blanc
    /*quand on appelle cette méthode avec dessine_sur() dans demarre() de systeme,
    on a normalement que la résolution de laplace et donc l'update du ciel sont déjà fait */
    QMatrix4x4 matrice;
...
void VueOpenGL::dessine(Ciel const& a_dessiner){
    ...
    a_dessiner.applique_fonction([&](const CubedAir& cba, std::size_t i, std::size_t j, std::size_t k){
        if(cba.nuage()){
            double couleur = 0.6+random()/8+k/250.;
            dessineSphere(a_dessiner.coordonnees(i,j,k)+0.5*lambda*Vecteur3D(1), 0.71*lambda, false, couleur*Vecteur3D(1,1,1)); // directement mis en x,y,z
        }
        if((qte=cba.pluie())){
             dessinePluie(a_dessiner.coordonnees(i,j,k), qte); // coordonnees() pour directement mis en x,y,z
        }
    });
}
...
void VueOpenGL::dessineFace(Vecteur3D const& s1, Vecteur3D const& s2, Vecteur3D const& s3, double seuil){
    Vecteur3D ortho((s2-s1)^(s3-s1));
    double lum(0.6-(~ortho)*soleil/2);
    Vecteur3D green{0.13, 0.54, 0.13}, brown{0.5, 0.3, 0.2};

    QMatrix4x4 point_de_vue;
    prog.setUniformValue("vue_modele", matrice_vue * point_de_vue);
    Vecteur3D c1 = lum*(s1[2]>seuil?brown:green),
              c2 = lum*(s2[2]>seuil?brown:green),
...
/*Vecteur3D{0.6, 0.3, 0.0}:Vecteur3D{0.13, 0.54, 0.13}),
