...
void ChampPotentiels ::  initialise (double vent, Montagne const& Everest) {
	boites.clear(); // pour garantir que un tableau non vide ne soit pas implementer de nouvelle valeur et que l'on puisse plus controler la taille de la boite
		...
		for (unsigned int j(0) ; j<nombre_de_cubes[1] ; ++j){
			...
			for (unsigned int k(0) ; k<nombre_de_cubes[2] ; ++k){
					...
					if(coordonnees(i,j,k)[2]>Everest.altitude(coord)){ //si c'est sous/sur la montagne 
						Potentiel p(-(vent/2)*coordonnees(i,j,k)[2],(vent/2)*coordonnees(i,j,k)[1]);
						P2.push_back(p);
...
for (unsigned int i(0) ; i<nombre_de_cubes[0]-1 ; ++i){
		for (unsigned int j(0) ; j<nombre_de_cubes[1]-1 ; ++j){
			for (unsigned int k(0) ; k<nombre_de_cubes[2]-1 ; ++k){
				e+= boites[i][j][k].get_Laplacien().norme2();
}
}
}
	if(e>erreur){		
...
Vecteur2D ChampPotentiels :: calcule_laplacien (unsigned int i, unsigned int j, unsigned int k) const {
	return boites[i-1][j][k].get_potentiel()+boites[i][j-1][k].get_potentiel()+boites[i][j][k-1].get_potentiel()-6*boites[i][j][k].get_potentiel()+boites[i+1][j][k].get_potentiel()+boites[i][j+1][k].get_potentiel()+boites[i][j][k+1].get_potentiel();
}
...
do {
	for (unsigned int i(0) ; i<nombre_de_cubes[0]-1 ; ++i){
		for (unsigned int j(0) ; j<nombre_de_cubes[1]-1 ; ++j){
			for (unsigned int k(0) ; k<nombre_de_cubes[2]-1 ; ++k){
				boites[i][j][k].access_Laplacien(calcule_laplacien(i,j,k));
			}
		}
	}
	for (unsigned int i(0) ; i<nombre_de_cubes[0]-1 ; ++i){
		for (unsigned int j(0) ; j<nombre_de_cubes[1]-1 ; ++j){
			for (unsigned int k(0) ; k<nombre_de_cubes[2]-1 ; ++k){
				boites[i][j][k].access_potentiel(boites[i][j][k].get_potentiel()+epsilon*boites[i][j][k].get_Laplacien());
			}
		}
	}
	++n;
...
void ChampPotentiels :: affiche() const {
	for (unsigned int i(0) ; i<nombre_de_cubes[0] ; ++i){
		for (unsigned int j(0) ; j<nombre_de_cubes[1] ; ++j){
			for (unsigned int k(0) ; k<nombre_de_cubes[2] ; ++k){
				cout<<i<<" "<<j<<" "<<k<<" "<<boites[i][j][k].get_potentiel()<<endl;
			}
		}
	}
}
