...
public:
    ...
    coeur(autre.coeur->copie())
  {
    for (auto const& p_acc : autre.accessoires) {
      accessoires.push_back(p_acc->copie());//copie() est polymorphique
    }
  }
  ...
  virtual void afficher(ostream& sortie) const override {
    ...
    sortie << "  * " << *coeur << endl;
    for (auto const& p_acc : accessoires) {
      sortie << "  * " << *p_acc << endl; //call operator<< (line 45); qui fait sortie<<prix()
    }									// avce prix()=meth polymorph ; qui return sortie (un ostream)
...
int main() {
  ...
  m += new Bracelet("cuir", 54.0); //operation possible grâce à opérateur +=
