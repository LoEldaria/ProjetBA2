...
bool Vecteur2D :: compare (Vecteur2D const& vec2)const{
    return (vecteur[0]==vec2.vecteur[0] and vecteur[1]==vec2.vecteur[1]);
}

const Vecteur2D Vecteur2D :: addition( Vecteur2D const& autre) const {
    Vecteur2D A;
    A.set_coord(0,autre.vecteur[0]+vecteur[0]);
    A.set_coord(1,autre.vecteur[1]+vecteur[1]);
    return A;
}

const Vecteur2D Vecteur2D :: soustraction ( Vecteur2D const& autre) const {
    Vecteur2D A;
    A.set_coord(0,vecteur[0]-autre.vecteur[0]);
    A.set_coord(1,vecteur[1]-autre.vecteur[1]);
    return A;
}
...
double Vecteur2D :: prod_scal(Vecteur2D const& autre) const{
    ...
    scalaire_result=autre.vecteur[0]*vecteur[0]+autre.vecteur[1]*vecteur[1];
