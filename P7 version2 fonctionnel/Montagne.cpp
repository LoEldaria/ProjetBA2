...
double Montagne ::altitude(array<unsigned int ,2> coord) const {
    double  x1 = max(coord[0],centre[0]) - min(coord[0],centre[0]),
            x2 = max(coord[1],centre[1]) - min(coord[1],centre[1]),
